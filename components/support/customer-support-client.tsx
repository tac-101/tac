			...
			<header className="border-b border-border">
				<div className="flex items-center justify-between px-6 lg:px-8 py-4 max-w-6xl mx-auto">
					<div className="flex items-center gap-3">
						<div className="h-10 w-10 rounded-lg bg-primary flex items-center justify-center text-xl font-display">
							📦
						</div>
				...
				<div className="w-full max-w-5xl grid gap-8 lg:gap-10 lg:grid-cols-[minmax(0,1.2fr)_minmax(0,1fr)] items-start">
					<section className="space-y-5 max-w-xl">
						<div className="inline-flex items-center gap-2 rounded-full border border-border/60 bg-background/60 px-3 py-1 text-xs md:text-xs tracking-[0.18em] uppercase text-muted-foreground">
							...
							<span>
								Shipment questions · Delivery issues · Northeast corridor
							</span>
						</div>
						...
						<ul className="text-xs md:text-xs text-muted-foreground space-y-1">
							<li>
								• Use the same email you used for your booking or enquiry.
							</li>
							<li>
								• Include a shipment or barcode reference if you have one.
							</li>
							<li>• Our team will follow up using the email you provide.</li>
						</ul>

						{ticketId && (
							<p className="text-xs text-chart-2 bg-chart-2/10 border border-chart-2/30 rounded-md px-3 py-2 mt-2">
								Ticket created with reference{" "}
								<span className="font-mono">{ticketId}</span>. You can mention
								this ID when talking to support.
							</p>
						)}
					</section>
								...
								>
									...
									<FormField
										...
										render={({ field }) => (
											<FormItem>
												<FormLabel>
													Shipment or barcode reference (optional)
												</FormLabel>
												<FormControl>
													<Input
