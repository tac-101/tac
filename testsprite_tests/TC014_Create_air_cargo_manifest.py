...
async def run_test():
    ...
    try:
        ...
        # -> Click on 'Access Portal' button to log in as authorized user.
        frame = context.pages[-1]
        # Click on 'Portal' button to access login or portal page.
        ...
        # -> Dismiss or handle the new UI element or popup, then click Sign In button.
        frame = context.pages[-1]
        ...
        # -> Check if there is an option to reset password or request access, or retry login with corrected credentials.
        frame = context.pages[-1]
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Manifest creation failed due to invalid input').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test case failed: Manifest creation did not succeed as expected. The test plan execution has failed, indicating the manifest grouping by origin and destination with airline and flight details was not created or updated successfully.")
