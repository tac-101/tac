...
export default async function InvoicePublicPage({
	...
	if (Array.isArray(invoiceItems) && invoiceItems.length > 0) {
		...
		lineItems = allItems.map((item, index) => {
			...
			if (shipment?.origin || shipment?.destination) {
				const route =
					`${shipment.origin ?? ""} → ${shipment.destination ?? ""}`.trim();
				if (route) parts.push(route);
			}

			const description = parts.join(" | ") || `Shipment ${index + 1}`;
	...
	return (
		<div className="min-h-screen bg-muted/30 flex items-start justify-center py-10 px-4">
			<div className="w-full max-w-3xl bg-card shadow-xl rounded-lg overflow-hidden border border-border">
					...
					<div className="flex items-center gap-2">
						{pdfUrl && (
							<a
								...
								className="px-4 py-2 rounded bg-primary-foreground text-primary text-xs font-semibold shadow-sm hover:bg-primary-foreground/90"
							>
								Download
							</a>
						)}
				...
				<div className="px-6 py-4 border-b flex flex-col md:flex-row md:items-center md:justify-between gap-4">
					<div>
						<h1 className="text-lg font-semibold tracking-[0.15em] uppercase text-foreground">
							{companyProfile.name}
						</h1>
				...
				<div className="px-6 py-4 border-b grid grid-cols-1 md:grid-cols-2 gap-4 text-xs text-foreground">
					<div className="space-y-3">
						<div>
							<div className="uppercase tracking-[0.18em] text-xs text-muted-foreground mb-1">
								Bill To
							</div>
							...
							<tbody>
								{lineItems.length > 0 ? (
									...
									<tr>
										<td
											colSpan={4}
											className="py-3 px-2 text-center text-muted-foreground text-xs"
										>
				...
				<div className="px-6 py-4 border-b grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
					...
					<div className="md:text-right space-y-1 text-foreground/80">
						<div className="text-xs uppercase tracking-[0.18em] text-muted-foreground mb-2 md:text-right text-left">
							Summary
						</div>
						<div>
				...
				<div className="px-6 py-4 flex items-end justify-between text-xs text-muted-foreground">
					<div>
						<div>For: {companyProfile.name}</div>
						<div className="mt-6 border-t border-border w-40 pt-1 text-center">
