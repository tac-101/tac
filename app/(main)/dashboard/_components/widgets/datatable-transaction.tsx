...
export const columns: ColumnDef<Item>[] = [
    ...
    },
    {
        ...
        cell: ({ row }) => (
            <div className="flex items-center gap-2">
                {/* Replaced img with text/icon placeholder since we don't have the CDN assets locally/reliably */}
                {row.getValue('paidBy') === 'mastercard' ? (
                    <div className="h-6 w-10 bg-slate-100 rounded flex items-center justify-center text-[10px] font-bold text-slate-600">MC</div>
                ) : (
                    <div className="h-6 w-10 bg-blue-100 rounded flex items-center justify-center text-[10px] font-bold text-blue-600">VISA</div>
                )}
            </div>
        )
    },
