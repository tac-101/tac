...
export default function AdminSettingsPage() {
	...
	const handleToggleMaintenance = async (next: boolean) => {
		...
		try {
			...
			setSettings((prev) =>
				prev ? { ...prev, maintenanceMode: next } : { maintenanceMode: next },
			);
			...
			toast({
				...
				description:
					err?.message ||
					"Something went wrong while updating maintenance mode.",
				variant: "destructive",
			});
		} finally {
			setSaving(false);
			...
			<div className="space-y-6">
						...
						<div className="flex items-center justify-between">
							<div>
								...
								<p className="text-xs text-muted-foreground max-w-md">
									When enabled, operators should avoid starting new large jobs
									while you perform system maintenance or schema changes.
								</p>
							</div>
				...
				<Card className="border-pop bg-background">
					...
					<CardContent className="space-y-2 text-xs text-muted-foreground">
						<p>
							This section can be extended to surface environment and
							integration status (Supabase project, WhatsApp integration,
							storage, etc.).
						</p>
						<p>
						...
						<p>
							...
							depend on the <code>role</code> column in{" "}
