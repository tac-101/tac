...
export default function WarehouseManagement() {
	...
	const [editingWarehouse, setEditingWarehouse] = useState<UIWarehouse | null>(
		null,
	);
	const [updatingStatusId, setUpdatingStatusId] = useState<string | null>(null);
	const [userRole, setUserRole] = useState<string | null>(null);
	const [_roleLoaded, setRoleLoaded] = useState(false);
	const { toast } = useToast();
	const { setModuleContext } = useTapanAssociateContext();

	// Location context for filtering
	const { locationScope, getLocationFilter, isViewingAll, scopeLabel } =
		useLocation();
	...
	const handleDeleteWarehouse = async (warehouse: UIWarehouse) => {
		...
		try {
			...
			setWarehouseData((prev) => prev.filter((w) => w.id !== warehouse.id));
	...
	const handleUpdateWarehouseStatus = async (
		...
		try {
			...
			setWarehouseData((prev) =>
				prev.map((w) =>
					w.id === warehouseId ? { ...w, status: nextStatus } : w,
				),
			);
