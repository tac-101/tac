...
async function getShipmentData(awb: string) {
	...
	const { data: scans } = await supabaseAdmin
		.from("package_scans")
		.select("*")
		.in("barcode_id", shipment.barcodes?.map((b: any) => b.id) || [])
		...
		<div className="min-h-screen bg-gradient-to-br from-background via-background to-muted/20">
			...
			<div className="border-b border-border/50 bg-background/90 backdrop-blur-sm sticky top-0 z-10">
				<div className="container mx-auto flex h-16 max-w-6xl items-center justify-between px-4">
					<UnifiedLogo className="h-12 md:h-14 lg:h-16" />
										...
										{shipment.barcodes?.[0] && (
											<div>
												...
												<dd className="font-mono text-xs">
													{shipment.barcodes[0].barcode_number}
												</dd>
											</div>
										)}
										<div>
							...
							<GlassCard variant="subtle" className="bg-muted/30">
								<GlassCardContent className="p-6">
									<p className="text-xs text-muted-foreground">
										Need help? Contact us at support@tapango.com or call
										+91-XXX-XXX-XXXX
									</p>
								</GlassCardContent>
							</GlassCard>
